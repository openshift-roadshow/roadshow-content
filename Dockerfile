...
ENV WORKSHOPS_URLS="file:///opt/data/workshopper-workshops/roadshow.yml"
...
RUN yum -y install git && mkdir /opt/data && cd /opt/data && \
    ...
    git clone https://github.com/osevg/workshopper-workshops.git && \
    rm -rf workshopper-content/.git && rm -rf workshopper-workshops/.git && \
